...
uint64_t Engine::scan_directory(const std::filesystem::path& scan_root,
    ...
    core::Pipeline pipeline(*database_, working_dir_);
...
std::optional<ImageBuffer> Engine::get_preview(int64_t photo_id, uint32_t max_width, uint32_t max_height) {
    ...
    if (!std::filesystem::exists(full_source)) {
        spdlog::warn("Source file does not exist for preview: {}", full_source.string());
        return std::nullopt;
    }

    auto full_img = codec::ImageCodec::decode_file(full_source);
    if (full_img.empty()) {
        spdlog::warn("Failed to decode source image for preview: {}", full_source.string());
        return std::nullopt;
    }
